...
# Rscript viper_cond1cond2_networks_proteoJohansson.R
...
samp_annot_all <- setNames(as.character(annot_dt[,paste0("PAM50.subtype_merged")]), as.character(annot_dt$Tumor.ID))
...
write.table(shadow_summary, file = outFile, col.names=T, row.names=F, sep="\t", quote=F)
cat(paste0("... written: ", outFile, "\n"))
...
heatmap(as.matrix(cond12_vpres_dd), 
...
outFile <- file.path(outFolder, paste0(cond1, "_vs_", cond2, "_vprGeneSimilarityHeatmap_viz.", plotType))
do.call(plotType, list(outFile, height=myHeight*1.8, width=myWidth*1.8))
heatmap(as.matrix(as.dist(cond12_dd)), Rowv = as.dendrogram(hclust(as.dist(cond12_dd),
